...
public sealed class KindClusterScenarioHarness : IClusterScenarioHarness
{
    ...
    {
        await KindCluster.DownloadClient(cancellationToken).ConfigureAwait(false);
        await KindCluster.CreateCluster(_name, kubeConfigPath: _kubeConfigPath, cancellationToken: cancellationToken).ConfigureAwait(false);
        _clusterCreated = true;

        KubeConfig = await KindCluster.GetK8SConfiguration(_name, cancellationToken).ConfigureAwait(false);
    ...
    private async Task DeleteClusterAsync()
    {
        ...
        for (int attempt = 1; attempt <= 3; attempt++)
        {
            try
            {
                await KindCluster.DeleteCluster(_name, timeout.Token).ConfigureAwait(false);
