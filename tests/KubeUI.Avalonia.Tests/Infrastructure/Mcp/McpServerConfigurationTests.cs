...
public sealed class McpServerConfigurationTests
{
    ...
    [Fact]
    public void endpoint_prefers_bound_port_over_configured_port()
    {
    ...
    [Fact]
    public void endpoint_falls_back_to_configured_port_without_bound_port()
    {
        var settings = new Settings { McpServerEnabled = true, McpServerPort = 62888 };
    ...
    [Fact]
    public void state_records_bound_port()
    {
        var state = new McpServerState();
        state.BoundPort.ShouldBeNull();

        state.SetBoundPort(12345);

        state.BoundPort.ShouldBe(12345);
    }
