...
namespace KubeUI.Core.Components
{
    public partial class ResizePanel : IDisposable
    {
        ...
        {
            decimal elementY = await module.InvokeAsync<decimal>("getElementY", ReferenceToDiv);
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                module = await JS.InvokeAsync<IJSObjectReference>("import", $"./_content/KubeUI.Core/Components/{GetType().Name}.razor.js");
