...
public sealed class OperatorPrometheusProvider : PrometheusProviderBase
{
    public override PrometheusProviderKind Kind => PrometheusProviderKind.Operator;

    public override string Name => "Prometheus Operator";
    ...
    public override async Task<ResolvedPrometheusEndpoint?> TryResolveServiceAsync(k8s.Kubernetes client, ClusterMetricsSettings settings, CancellationToken cancellationToken = default)
    {
        if (!string.IsNullOrWhiteSpace(settings.PrometheusServiceNamespace)
            ...
            && settings.PrometheusServicePort is > 0)
        {
        ...
        var service = await FindFirstServiceForAllNamespacesAsync(client, "operated-prometheus=true", cancellationToken).ConfigureAwait(false);
