...
public sealed partial class Cluster : ObservableObject, ICluster
{
    ...
    private async Task AddDefaultNavigation()
    {
        ...
        {
            NavigationItems.Add(new NavigationLink() { Name = "Settings", ViewModelKey = NavigationTargets.ClusterSettings, Cluster = this, FluentIcon = Icon.Settings, Order = 0 });
            NavigationItems.Add(new NavigationLink() { Name = Assets.Resources.ClusterViewModel_Title, ViewModelKey = NavigationTargets.ClusterWorkspace, Cluster = this, SvgIcon = "/Assets/kube/infrastructure_components/unlabeled/control-plane.svg", Order = 1 });
            NavigationItems.Add(new NavigationLink() { Name = Assets.Resources.VisualizationViewModel_Title, ViewModelKey = NavigationTargets.Visualization, Cluster = this, FluentIcon = Icon.SearchVisual, Order = 2 });
        ...
        foreach (var config in ResourceConfigs)
        {
                ...
                (CanI(config.Value.Type, Verb.List) && CanI(config.Value.Type, Verb.Watch)))
            {
                ...
                if (config.Value.Type == typeof(V1Pod))
                {
                    ...
                    {
                        await Dispatcher.UIThread.InvokeAsync(() => networkNavItem.NavigationItems.Add(new NavigationLink() { Name = Assets.Resources.PortForwarderListViewModel_Title, ViewModelKey = NavigationTargets.PortForwarders, Cluster = this, FluentIcon = Icon.CloudFlow, Order = 6 }), DispatcherPriority.Background);
                    }
                }
