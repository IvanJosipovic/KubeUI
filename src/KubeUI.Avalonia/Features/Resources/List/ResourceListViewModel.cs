...
public partial class ResourceListViewModel<T> : ViewModelBase, IInitializeCluster, IDisposable, IResourceListViewModel where T : class, IKubernetesObject<V1ObjectMeta>, new()
{
    ...
    private const double FilterRowLabelMinWidth = 64;
    ...
    private DataGridColumnDefinition CreateTemplateColumnDefinition(IResourceListColumn columnDefinition, DataGridLengthConverter converter)
    {
        ...
        column.FilterFlyout = CreateFilterFlyout(columnDefinition, column);
        return column;
    }

    private FlyoutBase CreateFilterFlyout(IResourceListColumn columnDefinition, DataGridColumnDefinition column)
    {
        Flyout flyout;

        if (IsEnumType(columnDefinition.ValueType))
        {
            flyout = CreateEnumFilterFlyout(columnDefinition, column);
        }
        else if (IsDateType(columnDefinition.ValueType))
        {
            flyout = CreateDateFilterFlyout(columnDefinition, column);
        }
        else if (IsNumericType(columnDefinition.ValueType))
        {
            flyout = CreateNumericFilterFlyout(columnDefinition, column);
        }
        else
        {
            flyout = CreateTextFilterFlyout(columnDefinition, column);
        }
    ...
    private Flyout CreateTextFilterFlyout(IResourceListColumn columnDefinition, DataGridColumnDefinition column)
    {
        TextFilterFlyoutContext? context = null;
        context = new TextFilterFlyoutContext(
            columnDefinition.Name,
            apply: () => ApplyTextFilter(column, context!.SelectedOperator.Operator, context.Query),
            clear: () =>
            {
        ...
        var content = new StackPanel
        {
            ...
            Children =
            {
                ...
                BuildOperatorComboBox(context!),
                BuildTextBox(context!)
            }
        };

        content.Children.Add(BuildActionRow(context!.ApplyCommand, context.ClearCommand));
        content.DataContext = context;

        var flyout = new Flyout { Content = content };
        flyout.Opened += (_, _) => context.LoadFromDescriptor(GetCurrentFilterDescriptor(column));
        return flyout;
    }

    private Flyout CreateEnumFilterFlyout(IResourceListColumn columnDefinition, DataGridColumnDefinition column)
    {
        EnumFilterFlyoutContext? context = null;
        context = new EnumFilterFlyoutContext(
            columnDefinition.Name,
            columnDefinition.ValueType,
            apply: () => ApplyEnumFilter(column, context!.SelectedValue?.Value),
            clear: () =>
            {
                ClearColumnFilter(column);
                context!.ClearState();
        ...
        var content = new StackPanel
        {
            ...
            Children =
            {
                ...
                BuildLabeledEditorRow(Assets.Resources.ResourceListViewModel_Filter_Value, BuildEnumValueRow(context!))
            }
        };
    ...
    private Flyout CreateNumericFilterFlyout(IResourceListColumn columnDefinition, DataGridColumnDefinition column)
    {
        NumericFilterFlyoutContext? context = null;
        context = new NumericFilterFlyoutContext(
            columnDefinition.Name,
            apply: () => ApplyNumericFilter(column, context!.SelectedOperator.Operator, context.Value, context.SecondValue),
            clear: () =>
            {
                ClearColumnFilter(column);
                context!.ClearState();
        ...
        var content = new StackPanel
        {
            ...
            Children =
            {
                ...
                BuildLabeledEditorRow(string.Empty, BuildOperatorComboBox(context!)),
                BuildNumericValueRow(context!),
                BuildRangeValueRow(context!)
            }
        };
    ...
    private Flyout CreateDateFilterFlyout(IResourceListColumn columnDefinition, DataGridColumnDefinition column)
    {
        DateFilterFlyoutContext? context = null;
        context = new DateFilterFlyoutContext(
            columnDefinition.Name,
            apply: () => ApplyDateFilter(column, columnDefinition.ValueType, context!.SelectedOperator.Operator, context.Amount, context.SelectedUnit.Unit),
            clear: () =>
            {
                ClearColumnFilter(column);
                context!.ClearState();
    ...
    private static ComboBox BuildOperatorComboBox(TextFilterFlyoutContext context)
    {
        var comboBox = new ComboBox
        {
            Width = 220,
            ItemsSource = context.Operators,
            ItemTemplate = new FuncDataTemplate<FilterOperatorChoice>((item, _) =>
                new TextBlock { Text = item.Label })
        };

        comboBox.Bind(SelectingItemsControl.SelectedItemProperty, new Binding(nameof(TextFilterFlyoutContext.SelectedOperator))
        {
            Source = context,
    ...
    private static ComboBox BuildOperatorComboBox(NumericFilterFlyoutContext context)
    {
        var comboBox = new ComboBox
        {
            Width = 220,
        ...
        comboBox.Bind(SelectingItemsControl.SelectedItemProperty, new Binding(nameof(NumericFilterFlyoutContext.SelectedOperator))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });
    ...
    private static ComboBox BuildOperatorComboBox(DateFilterFlyoutContext context)
    {
        var comboBox = new ComboBox
        {
            Width = 220,
        ...
        comboBox.Bind(SelectingItemsControl.SelectedItemProperty, new Binding(nameof(DateFilterFlyoutContext.SelectedOperator))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });
    ...
    private static ComboBox BuildEnumValueRow(EnumFilterFlyoutContext context)
    {
        var comboBox = new ComboBox
        {
            Width = 220,
            ItemsSource = context.Options,
            ItemTemplate = new FuncDataTemplate<EnumFilterChoice>((item, _) =>
                new TextBlock { Text = item.Label })
        };

        comboBox.Bind(SelectingItemsControl.SelectedItemProperty, new Binding(nameof(EnumFilterFlyoutContext.SelectedValue))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });
    ...
    private static TextBox BuildTextBox(TextFilterFlyoutContext context)
    {
        var textBox = new TextBox
        {
            Width = 220,
            Watermark = Assets.Resources.ResourceListViewModel_Filter_Value,
            HorizontalAlignment = HorizontalAlignment.Left
        };

        textBox.Bind(TextBox.TextProperty, new Binding(nameof(TextFilterFlyoutContext.Query))
        {
            Source = context,
            Mode = BindingMode.TwoWay,
    ...
    private static Grid BuildNumericValueRow(NumericFilterFlyoutContext context)
    {
        var numericUpDown = new NumericUpDown { Width = 120, Increment = 1 };
        numericUpDown.Bind(NumericUpDown.ValueProperty, new Binding(nameof(NumericFilterFlyoutContext.Value))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });

        return BuildLabeledEditorRow(Assets.Resources.ResourceListViewModel_Filter_Value, numericUpDown);
    }

    private static Grid BuildRangeValueRow(NumericFilterFlyoutContext context)
    {
        var numericUpDown = new NumericUpDown { Width = 120, Increment = 1 };
        numericUpDown.Bind(NumericUpDown.ValueProperty, new Binding(nameof(NumericFilterFlyoutContext.SecondValue))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });

        var panel = BuildLabeledEditorRow(Assets.Resources.ResourceListViewModel_Filter_And, numericUpDown);
        panel.Bind(Visual.IsVisibleProperty, new Binding(nameof(NumericFilterFlyoutContext.IsRangeVisible))
        {
            Source = context
        });
    ...
    private static Grid BuildDateRelativeRow(DateFilterFlyoutContext context)
    {
        ...
        amount.Bind(NumericUpDown.ValueProperty, new Binding(nameof(DateFilterFlyoutContext.Amount))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });
        ...
        {
            Width = 120,
            ItemsSource = context.Units,
            ItemTemplate = new FuncDataTemplate<DateRelativeUnitChoice>((item, _) =>
                new TextBlock { Text = item.Label })
        };

        unit.Bind(SelectingItemsControl.SelectedItemProperty, new Binding(nameof(DateFilterFlyoutContext.SelectedUnit))
        {
            Source = context,
            Mode = BindingMode.TwoWay
        });
        ...
        return BuildLabeledEditorRow(Assets.Resources.ResourceListViewModel_Filter_Value, editor);
    }

    private static Grid BuildLabeledEditorRow(string label, Control editor)
    {
        ...
        var grid = new Grid
        {
            ...
            Children =
            {
                ...
                {
                    Text = label,
                    MinWidth = FilterRowLabelMinWidth,
    ...
    private static StackPanel BuildActionRow(ICommand applyCommand, ICommand clearCommand)
    {
        return new StackPanel
        {
            ...
            Children =
            {
                new Button
                {
                    Content = Assets.Resources.ResourceListViewModel_Filter_Clear,
                    Command = clearCommand
                },
                new Button
                {
                    Content = Assets.Resources.ResourceListViewModel_Filter_Apply,
    ...
    private FilteringDescriptor? GetCurrentFilterDescriptor(DataGridColumnDefinition column)
    {
    ...
    private void ClearColumnFilter(DataGridColumnDefinition column)
    {
        FilteringModel.Remove(column);
    }

    private void ApplyTextFilter(DataGridColumnDefinition column, FilteringOperator filterOperator, string? query)
    {
    ...
    private void ApplyEnumFilter(DataGridColumnDefinition column, object? value)
    {
        if (value == null)
        {
            ClearColumnFilter(column);
    ...
    private void ApplyNumericFilter(DataGridColumnDefinition column, FilteringOperator filterOperator, double? value, double? secondValue)
    {
        if (filterOperator == FilteringOperator.Between)
        {
            ...
            if (value != null && secondValue != null)
            {
                FilteringModel.SetOrUpdate(new FilteringDescriptor(
                    ...
                    propertyPath: null,
                    values: new object[] { value.Value, secondValue.Value }));
                return;
            }
    ...
    private void ApplyDateFilter(DataGridColumnDefinition column, Type valueType, FilteringOperator filterOperator, double? amount, DateRelativeUnit unit)
    {
    ...
    private static DateTimeOffset ComputeRelativeDateThreshold(double amount, DateRelativeUnit unit)
    {
    ...
    private sealed record FilterOperatorChoice(FilteringOperator Operator, string Label);

    private enum DateRelativeUnit
    {
        Minutes,
        Hours,
        Days,
    ...
    private sealed record DateRelativeUnitChoice(DateRelativeUnit Unit, string Label);

    private abstract class ColumnFilterFlyoutContextBase : ObservableObject
    {
    ...
    private sealed record EnumFilterChoice(string Label, object? Value);

    private sealed class EnumFilterFlyoutContext : ObservableObject
    {
        ...
        private static ObservableCollection<EnumFilterChoice> CreateOptions(Type enumType)
        {
            ...
            var items = new List<EnumFilterChoice> { new(Assets.Resources.ResourceListViewModel_Filter_Any, null) };
    ...
    private sealed class TextFilterFlyoutContext : ColumnFilterFlyoutContextBase
    {
        private string _query = string.Empty;

        public TextFilterFlyoutContext(string title, Action apply, Action clear)
            : base(title, s_textOperatorChoices, apply, clear)
        {
        }
    ...
    private sealed class NumericFilterFlyoutContext : ColumnFilterFlyoutContextBase
    {
        private double? _value;
        private double? _secondValue;

        public NumericFilterFlyoutContext(string title, Action apply, Action clear)
            : base(title, s_numericOperatorChoices, apply, clear)
        {
        }
    ...
    private sealed class DateFilterFlyoutContext : ColumnFilterFlyoutContextBase
    {
        ...
        public DateFilterFlyoutContext(string title, Action apply, Action clear)
            : base(title, s_dateOperatorChoices, apply, clear)
        {
            Units = new ObservableCollection<DateRelativeUnitChoice>(s_dateRelativeUnitChoices);
        ...
        private static RelativeAmount InferRelativeAmount(DateTimeOffset threshold)
        {
            ...
            {
                return new RelativeAmount(Math.Max(1, Math.Round(difference.TotalDays / 30d)), s_dateRelativeUnitChoices[3]);
            }

            if (difference.TotalDays >= 1)
            {
                return new RelativeAmount(Math.Max(1, Math.Round(difference.TotalDays)), s_dateRelativeUnitChoices[2]);
            }

            if (difference.TotalHours >= 1)
            {
                return new RelativeAmount(Math.Max(1, Math.Round(difference.TotalHours)), s_dateRelativeUnitChoices[1]);
            }

            return new RelativeAmount(Math.Max(1, Math.Round(difference.TotalMinutes)), s_dateRelativeUnitChoices[0]);
        }

        private readonly record struct RelativeAmount(double Amount, DateRelativeUnitChoice Unit);
    }

    private static readonly FilterOperatorChoice[] s_textOperatorChoices =
    [
        new(FilteringOperator.Contains, Assets.Resources.ResourceListViewModel_Filter_Contains),
        new(FilteringOperator.Equals, Assets.Resources.ResourceListViewModel_Filter_Equals),
        new(FilteringOperator.StartsWith, Assets.Resources.ResourceListViewModel_Filter_StartsWith),
        new(FilteringOperator.EndsWith, Assets.Resources.ResourceListViewModel_Filter_EndsWith),
    ];

    private static readonly FilterOperatorChoice[] s_numericOperatorChoices =
    [
        new(FilteringOperator.Between, Assets.Resources.ResourceListViewModel_Filter_Between),
        new(FilteringOperator.Equals, Assets.Resources.ResourceListViewModel_Filter_Equals),
        new(FilteringOperator.GreaterThan, Assets.Resources.ResourceListViewModel_Filter_GreaterThan),
        new(FilteringOperator.GreaterThanOrEqual, Assets.Resources.ResourceListViewModel_Filter_GreaterThanOrEqual),
        new(FilteringOperator.LessThan, Assets.Resources.ResourceListViewModel_Filter_LessThan),
        new(FilteringOperator.LessThanOrEqual, Assets.Resources.ResourceListViewModel_Filter_LessThanOrEqual),
    ];

    private static readonly FilterOperatorChoice[] s_dateOperatorChoices =
    [
        new(FilteringOperator.GreaterThan, Assets.Resources.ResourceListViewModel_Filter_NewerThan),
        new(FilteringOperator.LessThan, Assets.Resources.ResourceListViewModel_Filter_OlderThan),
    ];

    private static readonly DateRelativeUnitChoice[] s_dateRelativeUnitChoices =
    [
        new(DateRelativeUnit.Minutes, Assets.Resources.ResourceListViewModel_Filter_Minutes),
        new(DateRelativeUnit.Hours, Assets.Resources.ResourceListViewModel_Filter_Hours),
        new(DateRelativeUnit.Days, Assets.Resources.ResourceListViewModel_Filter_Days),
        new(DateRelativeUnit.Months, Assets.Resources.ResourceListViewModel_Filter_Months),
    ];
