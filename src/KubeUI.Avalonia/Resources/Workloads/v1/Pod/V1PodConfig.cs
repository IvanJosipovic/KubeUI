...
public sealed partial class V1PodConfig : ResourceConfigBase<V1Pod>
{
    ...
    private async Task ViewLogs(object? parameter)
    {
        ...
        else if (parameters is not null && parameters.Count == 2 && parameters[0] is V1Pod submenuPod)
        {
            pod = submenuPod;
    ...
    private static string GetDefaultContainerName(V1Pod pod)
    {
        ...
        k8s.Models.V1EphemeralContainer? ephemeralContainer = pod.Spec?.EphemeralContainers?.FirstOrDefault();
    ...
    private void ViewConsole(IList parameters)
    {
        ...
        string? containerName = GetContainerName(parameters[1]);
        if (containerName is null)
        {
            return;
        }
    ...
    private bool CanOpenConsole(IList? parameters, string subResource)
    {
        ...
        if (parameters?[0] is V1Pod pod && IsContainerTarget(parameters[1]))
        {
